//为最优线程数模型预测提取特征 注意topk分数分为查询的，链的，shard的 check ok
#include<iostream>
#include<vector>
#include<stdint.h>
#include<stdio.h>
...
{
	FILE *file = fopen((filename + ".score").c_str(), "rb");//score是加入q_weight的
	unsigned length = 0;
...
void read_TermtopKThresh(string filename)
{
	...
	for (int i = 0; i<queries.size(); i++)
	{
		...
		queryFeatures[i].query_topK_score = score;//为了保证凑满topk
